...
def main():
    timeTaken = time()
    comparisonImg = Image.new(mode='RGB', size=(
        2560, 2560), color=(255, 255, 255))
    ...
    for index, line in enumerate(lines):
        ...
        if startTxt.find("color") == -1 or line.find("{") == -1:
            continue
        colour = line[line.find("{"): line.find("}") +
                      1].replace("{", "[").replace("}", "]")
    ...
    while True:
        ...
        for index, line in enumerate(newlines[:-1]):
            ...
            if startTxt[0] == "{" and startTxtNext[0] == "}":
                ...
                # Classes than inherit must have brackets
                if len(classLine.strip().split(" ")) == 3:
                    classLine = classLine[:strIndex] + \
                        " {};" + classLine[strIndex:]
                else:
                    classLine = classLine[:strIndex] + \
                        ";" + classLine[strIndex:]
        ...
        for index, line in enumerate(newlines):
            ...
            if lineWords[0] != "class":
                continue  # Only interested in removing classes
            classType = lineWords[1].replace(":", "")
            if line.find(";") == -1:
                continue  # If line does not contain semicolon then the class contains data
            classType = classType.replace(";", "")
            numOfReferences = len(
                [l for l in newlines if ": "+classType+"\n" in l or ": "+classType+";" in l])
        ...
        if deleteLines == []:
            break
    print("Simplified classes in {0} iterations and {1}s".format(
        iterations, round(time() - timeTaken, 2)))
    timeTaken = time()
    ...
    for index, line in enumerate(newlines):
        ...
        if lineWords[0] != "class":
            continue
        if lineWords[1][-1] != ":":
            continue  # Is class inheriting?
        inheritance = lineWords[2].replace(";", "")
        declarations = [line for line in newlines if line.find(
            "class " + inheritance) != -1]
        if len(declarations) == 0:
            warning = "WARNING: class {0} is inherited but never declared".format(
                inheritance)
